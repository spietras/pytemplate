...
from pytemplate import resources


def extract_resource_path(resource_path: Union[str, PurePath]) -> Tuple[str, str]:
    ...
    if not resource_path.is_relative_to(PurePath('.')):
        raise ValueError(f"Path has to be relative to current dir, but '{resource_path}' is not")
    if ".." in resource_path.parts:
        raise ValueError(f"Path can't go backwards, but '{resource_path}' does")
...
def resource(resource_path: Union[str, PurePath]) -> ContextManager[Path]:
    ...
    package_part = f"{resources.__name__}{'.' + package_part if package_part else ''}"
...
def resource_text(resource_path: Union[str, PurePath], encoding: str = None, errors: str = None) -> str:
